describe('config module', () => {
    const validEnv = {
      ...
      ECOMMERCE_IO_CLIENT_REDIRECT_OUTCOME_PATH: 'http://localhost:1234/io-outcome',
      ECOMMERCE_CHECKOUT_CLIENT_REDIRECT_OUTCOME_PATH: 'http://localhost:1234/checkout-outcome',
    ...
    it('getConfig() should decode a valid environment', () => {
      ...
      if (result._tag === 'Right') {
        ...
        expect(result.right.ECOMMERCE_SHOW_CONTINUE_IO_BTN_DELAY_MILLIS).toBe(2000);
      }
    });
  
    it('getConfig() should return Left when required vars are missing', () => {
      // @ts-ignore
      window._env_ = { ECOMMERCE_ENV: '' };
      const { getConfig } = require('../config');
      const result = getConfig();
