...
describe("ecommerceIOPostTransaction", () => {
  ...
  beforeEach(() => {
    ...
    (getSessionItem as jest.Mock).mockImplementation((key) => {
      if (key === SessionItems.rptId) return mockRptId;
  ...
  it("returns Some(response) when transaction succeeds with status 200", async () => {
    ...
    (ecommerceIOClientWithPollingV1.newTransactionForEcommerceWebview as jest.Mock).mockResolvedValue(E.right(mockResponse));
  ...
  it("returns None when the client throws an error", async () => {
    (ecommerceIOClientWithPollingV1.newTransactionForEcommerceWebview as jest.Mock).mockRejectedValue(new Error("Network error"));
