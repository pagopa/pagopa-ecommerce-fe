import {
  ...
  import { ViewOutcomeEnum, PaymentGateway, NpgResultCodeEnum, RedirectResultCodeEnum } from '../types';
  ...
  describe('authorizationStatusMap', () => {
    it('contains expected codes and outcomes', () => {
      ...
      expect(authorizationStatusMap.get('101')).toBe(ViewOutcomeEnum.INVALID_CARD);
      expect(authorizationStatusMap.get('116')).toBe(ViewOutcomeEnum.BALANCE_LIMIT);
  ...
  describe('wasAuthorizedByGateway()', () => {
    it('returns true for NPG with EXECUTED', () => {
      expect(
        wasAuthorizedByGateway({ gateway: PaymentGateway.NPG, authorizationStatus: NpgResultCodeEnum.EXECUTED })
      ).toBe(true);
    });
    it('returns false for NPG with non-EXECUTED', () => {
      expect(
        wasAuthorizedByGateway({ gateway: PaymentGateway.NPG, authorizationStatus: NpgResultCodeEnum.PENDING })
      ).toBe(false);
    });
    it('returns true for REDIRECT with OK', () => {
      expect(
        wasAuthorizedByGateway({ gateway: PaymentGateway.REDIRECT, authorizationStatus: RedirectResultCodeEnum.OK })
      ).toBe(true);
    });
    it('returns false for REDIRECT with non-OK', () => {
      expect(
        wasAuthorizedByGateway({ gateway: PaymentGateway.REDIRECT, authorizationStatus: RedirectResultCodeEnum.ERROR })
      ).toBe(false);
    });
  ...
  describe('getOnboardingPaymentOutcome()', () => {
    const baseInfo = {
      gatewayInfo: { gateway: PaymentGateway.NPG, authorizationStatus: NpgResultCodeEnum.EXECUTED },
      nodeInfo: {} as any,
    };
  
    it('returns SUCCESS on NOTIFIED_OK', () => {
      expect(
        getOnboardingPaymentOutcome({ status: TransactionStatusEnum.NOTIFIED_OK, ...baseInfo })
      ).toBe(ViewOutcomeEnum.SUCCESS);
    });
  
    it('returns GENERIC_ERROR for REFUND_ERROR', () => {
      expect(
        getOnboardingPaymentOutcome({ status: TransactionStatusEnum.REFUND_ERROR, ...baseInfo })
    ...
    it('handles closed with NOT_RECEIVED as TAKE_IN_CHARGE', () => {
      expect(
        getOnboardingPaymentOutcome({
          ...
          nodeInfo: { sendPaymentResultOutcome: SendPaymentResultOutcomeEnum.NOT_RECEIVED },
        })
  ...
  describe('closePaymentResultError branch', () => {
    it('delegates to evaluateClosePaymentResultError when authorized', () => {
      ...
      const outcome = getOnboardingPaymentOutcome({
        status: TransactionStatusEnum.CLOSED,
        gatewayInfo: { gateway: PaymentGateway.NPG, authorizationStatus: NpgResultCodeEnum.EXECUTED },
        nodeInfo: { closePaymentResultError: { statusCode: 422, description: 'Node did not receive RPT yet' } },
      });
    ...
    it('falls back to GENERIC_ERROR if no match', () => {
      const outcome = getOnboardingPaymentOutcome({
        status: TransactionStatusEnum.CLOSED,
        gatewayInfo: { gateway: PaymentGateway.NPG, authorizationStatus: NpgResultCodeEnum.EXECUTED },
        nodeInfo: { closePaymentResultError: { statusCode: 599, description: 'other' } },
      });
    ...
    it('calls evaluateUnauthorizedStatus when not authorized', () => {
      const outcome = getOnboardingPaymentOutcome({
        status: TransactionStatusEnum.CLOSED,
        gatewayInfo: { gateway: PaymentGateway.REDIRECT, authorizationStatus: RedirectResultCodeEnum.ERROR },
        nodeInfo: { closePaymentResultError: { statusCode: 400, description: 'ignored' } },
      });
  ...
  describe('io-ts enum codecs', () => {
    it('EcommerceInterruptStatusCodeEnumType decodes valid value', () => {
      const r = EcommerceInterruptStatusCodeEnumType.decode(TransactionStatusEnum.REFUNDED as any);
    ...
    it('EcommerceMaybeInterruptStatusCodeEnumType decodes valid value', () => {
      const r = EcommerceMaybeInterruptStatusCodeEnumType.decode(TransactionStatusEnum.CLOSURE_ERROR as any);
      expect(r._tag).toBe('Right');
    });
  });
